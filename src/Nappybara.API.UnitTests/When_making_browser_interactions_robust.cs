...
namespace Nappybara.API.UnitTests
{
	...
	public class When_making_browser_interactions_robust
	{
		...
		{
			var robustness = new WaitAndRetryRobustness(TimeSpan.FromMilliseconds(10), TimeSpan.FromMilliseconds(5));
			var tries = 0;
			Action action = () =>
			                	{
			                		tries++;
		...
		public void When_an_Action_throws_a_recurring_exception_It_should_retry_until_the_timeout_is_reached_then_rethrow()
		{
			...
			var robustness = new WaitAndRetryRobustness(expectedTimeout, interval);
		...
		public void When_an_Action_throws_a_recurring_exception_It_should_retry_at_regular_intervals()
		{
			...
			var robustness = new WaitAndRetryRobustness(timeout, interval);
